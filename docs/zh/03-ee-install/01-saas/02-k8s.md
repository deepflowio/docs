...
::: warning
注意：如果部署 K8s 集群的云服务器已经通过云平台对接自动学习并记录到某个云平台，那么该 K8s 集群必须要被视为该云平台的`附属容器集群`，否则将导致 DeepFlow 的可观测性数据打标时无法确定数据所属的云平台，从而产生数据标签的紊乱。
:::
...
![获取 deepflowK8sClusterID（容器集群 ID）](https://yunshan-guangzhou.oss-cn-beijing.aliyuncs.com/pub/pic/2024070366850475cb2db.png)
