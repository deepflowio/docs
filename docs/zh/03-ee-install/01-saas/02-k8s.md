...
对于独立容器集群，可以由 DeepFlow Server 在收到该集群的首个 DeepFlow Agent 注册信息后自动创建，无需运维人员操作，可直接进入第 2 节。
...
对部分公有云 K8s 产品，DeepFlow 能够通过公有云 API 自动学习并录入 K8s 集群信息，无需运维人员操作，自动学习并录入的附属容器集群列表可以在 DeepFlow Web 页面的「资源」-「资源池」-「云平台」中查看。
