...
DeepFlow 平台对所有观测数据自动化注入`云资源`、`K8s 资源`和`K8s 自定义 Label`标签，这些标签是 DeepFlow 可观测性分析的基础，因此 DeepFlow 在开始工作之前需首先完成云、容器各类资源信息的学习。
...
> 注：如果部署 K8s 集群的云服务器已经通过云平台对接自动学习到某个云平台，那么该 K8s 集群必须要对接或录入为附属容器集群，否则将导致 DeepFlow 的可观测性数据打标时，无法确定数据所属的云平台，从而产生数据标签的紊乱。
