...
**（2）新建采集器组并获取 `agent-group-id`**

   `agent-group-id`即采集器组 ID，操作步骤参考下图：
...
- `deepflowServerNodeIPS`，即 DeepFlow Agent 所要连接的 DeepFlow Server 地址，与下面样例中的地址保持一致；当此处配置的地址错误时，DeepFlow Agent 将无法注册；
