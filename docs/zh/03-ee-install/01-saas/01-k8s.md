...
- **新建采集器组并获取 `agent-group-id`**
   ...
   `agent-group-id`即`采集器组 ID`，操作步骤参考下图：
...
- **生成 DeepFlow Agent 的配置文件**

新建一个名为 `values-custom.yaml` 的文件，并配置正确的 `agent-group-id`、`teamId`、`clusterNAME`
