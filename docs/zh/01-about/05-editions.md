...
- Universal Map（AutoMetrics）
  ...
  - 基于 eBPF/cBPF，自动采集 HTTP1/2、HTTPS（Golang/openssl）、Dubbo、gRPC、SOFARPC、FastCGI、MySQL、PostgreSQL、Redis、MongoDB、Kafka、MQTT、DNS 等应用调用日志
...
- 更丰富的 AutoMetrics 和 AutoTracing 数据
  - Agent 支持运行于多租户 Serverless K8s 节点、Windows Host 上
...
|                      | 提供 SQL、PromQL API                      | ✔      | ✔      |
|                      | 提供 OpenTelemetry OTLP Exporter API      | ✔      | ✔      |
