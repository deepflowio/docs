...
export default {
  ...
  methods: {
    ...
    generateCopyButton (parent) {
      ...
      copyElement.title = window.location.href.includes('/zh/') ? ZH_TOOL_TIP_MESSAGE : EN_TOOL_TIP_MESSAGE,
        copyElement.addEventListener('click', () => {
          this.copyToClipboard(parent.querySelector('pre').innerText)
        })
    ...
    copyToClipboard (str) {
      ...
      message.show({
        text: window.location.href.includes('/zh/') ? ZH_COPY_MESSAGE : EN_COPY_MESSAGE,
        duration: DURATION
      })
