...
- [1] The protocol header fields that need to be additionally collected can be defined in the static_config.l7-protocol-advanced-features.extra-log-fields of the collector configuration. For example, when User-Agent and Cookie are added to the configuration, the fields attribute.user_agent and attribute.cookie can be viewed in the call logs.
...
| response_length    | 响应长度       | --                   | Content-Length        | --                                                    |
| log_count          | 日志总量       | --                   | --                    | --                                                    |
| error              | 异常           | --                   | --                    | Client Exception + Server Exception                   |
...
|          | request_id         | 请求 ID      | correlation_id           | --                  | correlation_id high 32 bits of 64-bit integer |
...
**Metrics Field Mapping Table, the following table only includes fields with a mapping relationship**
