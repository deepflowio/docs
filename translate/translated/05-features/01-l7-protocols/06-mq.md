...
|          | response_status    | 响应状态     | --                        | error_code                | Normal: error_code=0; Server error: error_code!=0                                                                                                                                                                          |
...
| response_length    | 响应长度       | --           | message_size | --                                                          |
| log_count          | 日志总量       | --           | --           | --                                                          |
...
|          | request_id         | 请求 ID      | --             | --              | --                                                               |
|          | endpoint           | 端点         | topic          | --              | --                                                               |
| Resp.    | response_code      | 响应码       | --             | code            | Only for `connect_ack` messages to get the code                  |
|          | response_status    | 响应状态     | --             | code            | Normal: code=0; Client error: code=1/2/4/5; Server error: code=3 |
...
| log_count          | 日志总量       | --      | --       | --                                                          |
| error              | 异常           | --      | --       | Client error + Server error                                 |
| client_error       | 客户端异常     | --      | code     | Refer to the description of the Tag field `response_status` |
| server_error       | 服务端异常     | --      | code     | Refer to the description of the Tag field `response_code`   |
| error_ratio        | 异常比例       | --      | --       | Error / Response                                            |
| client_error_ratio | 客户端异常比例 | --      | --       | Client error / Response                                     |
| server_error_ratio | 服务端异常比例 | --      | --       | Server error / Response                                     |
...
Note: Due to protocol characteristics, currently only connections established after the agent starts are supported for AMQP protocol recognition.

Additionally, the following are unidirectional messages and will be directly saved as type=session call logs:
...
- Content-Body (`both`)
- Protocol-Header (`c->s`)
  The following messages may or may not have ACK, and DeepFlow uniformly ignores their responses (because ACK does not contain key information, and since they do not consistently ACK, there is no need to calculate latency):
- Basic.Publish (`c->s`)
- Basic.Deliver (`s->c`)
...
|          | response_status    | 响应状态     | --               | --                 | Normal: no error message; Server error: has error message                                                            |
...
|          | request_id         | 请求 ID      | --               | --               | --                                            |
|          | endpoint           | 端点         | subject          | --               | Only the part before the first `.` of subject |
| Resp.    | response_code      | 响应码       | --               | --               | --                                            |
|          | response_status    | 响应状态     | --               | --               | All considered normal                         |
|          | response_exception | 响应异常     | --               | --               | --                                            |
|          | response_result    | 响应结果     | --               | --               | --                                            |
...
Note, except for Info/Connect and Ping/Pong, other messages are unidirectional and will be directly saved as type=session call logs:
...
By parsing the [Pulsar](https://pulsar.apache.org/docs/3.2.x/client-libraries-python/) protocol, the fields of Pulsar Request/Response are mapped to the corresponding fields in the l7_flow_log. The mapping relationship is as follows:

**Tag Field Mapping Table, the following table only includes fields with a mapping relationship**

| Category | Name               | Chinese      | Request Header      | Response Header  | Description                                                                                                                                        |
| -------- | ------------------ | ------------ | ------------------- | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Req.     | version            | 协议版本     | protocol_version    | --               | Take the smaller one between CommandConnect and CommandConnected                                                                                   |
|          | request_type       | 请求类型     | command             | --               | --                                                                                                                                                 |
|          | request_domain     | 请求域名     | proxy_to_broker_url | --               | In CommandConnect                                                                                                                                  |
|          | request_resource   | 请求资源     | topic               | --               | Take the content after the last "/" in the topic                                                                                                   |
|          | request_id         | 请求 ID      | request_id          | --               | For Send/SendError/SendReceipt, since commands have no request_id, use the lower 16 bits of producer_id and sequence_id concatenated as request ID |
|          | endpoint           | 端点         | topic               | --               | --                                                                                                                                                 |
...
Note that the following are one-way messages and will be directly saved as type=session call logs:
...
By parsing the [ZMTP](https://rfc.zeromq.org/spec/23/) protocol (the message transport protocol used by ZeroMQ), the fields of ZMTP Request/Response are mapped to the corresponding fields in the l7_flow_log. The mapping relationship is as follows:
...
| Resp.    | response_code      | 响应码   | --             | --              | --                                                  |
|          | response_status    | 响应状态 | --             | --              | Normal: no error message; Error: with error message |
|          | response_exception | 响应异常 | --             | error message   | --                                                  |
| Misc.    | --                 | --       | --             | --              | --                                                  |

- In the ZMTP protocol, only when one end socket is REQ/REP, a request message must wait for the response of the previous request before initiating a new one. Requests and responses will be aggregated into one session.
- Other types are currently identified as one-way messages and will be directly saved as type=session call logs.
