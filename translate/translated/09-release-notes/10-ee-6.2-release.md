...
- Usability improvements
  ...
  - Page operations
    ...
    - Add quick filter functionality to the left side of table pages
    ...
    - Support configuring metric templates for quick selection of commonly used metric sets in Panels
  ...
  - Page display
    ...
    - Optimize the text display in the search box fields, providing more helpful information
    - Display the values and description information of enum fields in data tables
    - Preemptively gray out the jump button when the target page is predicted to have no data
...
- AutoTagging
  ...
  - Support synchronization of Baidu Cloud Cloud Smart Network (CSN) resource information
...
- Usability
  ...
  - Support batch input of Gateway type host and its NIC information
  - Support unified setting of `additional routing interfaces` for all managed K8s clusters under a public cloud account
- Compatibility
...
- API
  - **Querier adds support for PromQL**
    ...
    - When querying Prometheus native metrics, support using DeepFlow AutoTagging automatically injected tags
  ...
  - Querier SQL API
    ...
    - Complete support for the AS keyword and return the original field name before AS in the results
    - When obtaining optional values for enum type Tag fields, return the corresponding description information for the values
...
- eBPF
  ...
  - Simplify the protocol recognition logic in eBPF code
- Compatibility
  ...
  - eBPF programs support using BTF files to automatically adapt kernel offsets
- Controller and data node management
  - Support ClickHouse nodes greater than the number of deepflow-server replicas
  - Support specifying domain name forms for controller or ingester addresses for deepflow-agent
...
- Collector management
  ...
  - When the ctrl_ip or ctrl_mac of the agent runtime environment changes, support automatically updating the corresponding information of the agent to avoid disconnection
  ...
  - Automatically delete Agents that have been disconnected for more than the vtap_auto_delete_interval, with the default deletion occurring after more than 1 hour of disconnection
- Debug
